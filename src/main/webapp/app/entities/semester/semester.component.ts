...
import { ITEMS_PER_PAGE, Principal, Semester } from '../../shared';
...
export class SemesterComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.semesterService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<Semester[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/semester'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Semester) {
        return item.id;
    }
