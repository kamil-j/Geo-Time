...
import { ITEMS_PER_PAGE, Principal, ClassType } from '../../shared';
...
currentAccount: any;
    ...
    loadAll() {
        this.classTypeService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<ClassType[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/class-type'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ClassType) {
        return item.id;
    }
    registerChangeInClassTypes() {
        this.eventSubscriber = this.eventManager.subscribe('classTypeListModification', (response) => this.loadAll());
    }
